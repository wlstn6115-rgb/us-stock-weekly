(function(root){
 ...
 function create({date,before,deposit,orders,reason,memo,snapshot},today){
   ...
   return {id:root.crypto.randomUUID(),schemaVersion:1,createdAt:new Date().toISOString(),tradeDate:date,portfolioBefore:p.before,contribution:deposit,assetScoresAtDecision:Object.fromEntries(P.ASSETS.map(a=>[a,(snapshot?.assets[map[a].toLowerCase()]||snapshot?.assets[map[a]])?.score??null])),pricesAtDecision:Object.fromEntries(P.ASSETS.map(a=>[a,(snapshot?.assets[map[a].toLowerCase()]||snapshot?.assets[map[a]])?.price??null])),modelVersion:snapshot?.modelVersion||'unavailable',scoreObservationDate:snapshot?.date||null,scoreAvailableAt:snapshot?.availableAt||null,actions:p.orders,portfolioAfter:p.after,reasonCategory:reason,reasonMemo:memo,futurePerformance:pending(date),snapshotQuality:snapshot?'captured':'unavailable'};
 }
