(function(root){
 ...
 const factorNames={equity_trend:'나스닥100 추세',relative_equity:'나스닥100/S&P500 상대강도',btc_trend:'비트코인 추세',gold_trend:'금 추세',btc_gold:'BTC/Gold',dollar:'달러인덱스',nominal_yield:'명목 10년 금리',liquidity:'연준 자산'};
 const number=n=>Number.isFinite(n)?n.toFixed(4):'—';
 let previous='';
 function formula(a,state){
   const score=state.result?.scores?.[a];if(!score)return '<p>계산 상세 자료가 없습니다.</p>';
   const terms=score.contributions||[],sum=terms.reduce((n,c)=>n+c.points,0);
   return `<details data-keep="formula-${a}"><summary>Score 계산식 · 현재 적용값</summary><p>기준일 ${esc(state.result.market_date)} · 현재 Score <strong>${number(score.current_score)}</strong> · 데이터 충족률 ${number(score.data_coverage*100)}%</p><p><code>Score = 50 + Σ(50 × 현재 가중치 × 방향 적용 정규화값)</code></p><p>현재 계산: 50 + (${number(sum)}) = <strong>${number(50+sum)}</strong></p><p>정규화값 = clip((변환값 − 과거 평균) / 과거 표준편차 / 3, −1, 1). 평균·표준편차에서 현재 관측은 제외합니다. 가격·비율은 로그 변화, 명목 금리는 차이를 사용합니다. 방향 적용값은 자산별 유불리 부호를 곱한 값입니다.</p><div class="table-wrap"><table><tr><th>요인</th><th>현재 가중치</th><th>방향 적용값*</th><th>산출식</th><th>기여점수</th><th>상태</th></tr>${terms.map(c=>`<tr><th>${esc(factorNames[c.factor]||c.factor)}</th><td>${number(c.weight*100)}%</td><td>${c.data_status==='ok'&&c.weight>0?number(c.points/(50*c.weight)):'—'}</td><td>50 × ${number(c.weight)} × ${c.data_status==='ok'&&c.weight>0?number(c.points/(50*c.weight)):'0 (미반영)'}</td><td>${number(c.points)}</td><td>${esc(c.data_status)}</td></tr>`).join('')}</table></div><p class="small">* 방향 적용값은 공개된 기여점수 ÷ (50 × 가중치)로 복원했습니다. 원본 지표 현재값·과거 평균·표준편차는 공개 데이터에 없어 표시하지 않습니다. 결측 요인은 기여 0으로 처리하며 다른 요인 가중치를 재정규화하지 않습니다. 표시는 반올림하여 작은 합계 차이가 있을 수 있습니다.</p><p class="small">모델 ${esc(AllocationModels.modelLabel(state.normalizedSnapshot?.modelVersion||state.result.config_hash))} · 계산 상세는 위 기준일의 값이며 과거 점의 계산식 상세는 저장되어 있지 않습니다.</p></details>`;
 }
