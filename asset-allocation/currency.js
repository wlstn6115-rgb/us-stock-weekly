(function(root){
  ...
  async function refresh(){try{const fx=await root.AllocationData.get().getFX();const age=Date.now()-Date.parse(fx.date+'T00:00:00Z');if(age>8*86400000||age<0)throw Error('최근 환율 갱신 필요');latest=fx;note.textContent=`현재 화면 환산: 1 USD = ${fx.value.toLocaleString('ko-KR')}원 (${fx.date} 관측). 입력은 원화, 시뮬레이션은 당시 환율을 사용합니다.`;}catch(e){latest=null;note.textContent='현재 자산 달러 환산: '+e.message+' · 원화 입력과 과거 환율 계산은 유지됩니다.';}repaint();}
