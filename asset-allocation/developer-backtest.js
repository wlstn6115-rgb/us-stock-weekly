(async function(){
 ...
 try{
  const response=await fetch('./backtest-versions.json',{cache:'no-store'});if(!response.ok)throw Error('버전 설정 조회 실패');versions=(await response.json()).versions;
  rows=await new HistoricalDataAdapter().read();if(rows.length<15)throw Error('월별 자료가 부족합니다.');
  ...
  $('data-info').textContent=`자료 ${rows[0].date} ~ ${rows.at(-1).date} · ${rows.length}개월 · 데이터 버전 ${rows[0].dataVersion}`;
