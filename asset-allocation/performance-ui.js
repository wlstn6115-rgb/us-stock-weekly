(function(root){
 ...
 root.renderPerformance=(session,target)=>{
   ...
   ].map(([title,render])=>`<tr><th>${title}</th>${report.rows.map(r=>`<td>${render(r)}</td>`).join('')}</tr>`).join('')}</tbody></table></div><p>월초 납입을 제거한 월별 수익률로 TWR·CAGR·MDD를 계산합니다. 변동성은 월수익률의 표본 표준편차 × √12입니다.</p><p>금액 손실은 납입금을 더해 조정한 이전 고점과 비교하며, 납입원금 대비 손실도 별도로 표시합니다. MDD에 최종 자산을 곱한 추정금액이 아닙니다.</p><p>월별 관측이므로 월중 급락은 반영되지 않습니다. 세금·수수료는 미반영입니다. 과거 연습 결과이며 미래 기대수익률이 아닙니다.</p>${report.benchmarkAvailable?'':'<p>이 상세 성과표는 사용자 결과입니다. S&P500 평가액과 납입금 비교는 상단 자산 트렌드에서 확인하세요.</p>'}`;
 };
})(globalThis);
