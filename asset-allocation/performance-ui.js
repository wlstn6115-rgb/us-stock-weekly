(function(root){
 const money=n=>n.toLocaleString('ko-KR')+'원',percent=n=>n===null?'—':(n*100).toFixed(2)+'%';
 root.renderPerformance=(session,target)=>{
   const report=root.PerformanceEngine.report(session);if(!report.completed){target.textContent='최종 성과 보고서는 연습 종료 후 표시됩니다.';return;}
   target.innerHTML=`<h3>최종 시뮬레이션 보고서</h3><div class="table-wrap"><table><thead><tr><th>지표</th>${report.rows.map(r=>`<th>${r.label}</th>`).join('')}</tr></thead><tbody>${[
